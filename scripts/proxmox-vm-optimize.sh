...
# Proxmox VM Post-Install Optimization Script
...
#
# Matrix Cluster Examples:
#   # Apply disk settings to Talos VMs
#   ./proxmox-vm-optimize.sh --name-contains talos-prod --apply
#
#   # Apply to specific VM IDs on Matrix
#   ./proxmox-vm-optimize.sh --vmids 100-106 --apply
#
#   # GPU passthrough (future use)
#   ./proxmox-vm-optimize.sh --vmids 200 --gpu --apply
#
# Note: Run this directly on a Proxmox node (Foxtrot, Golf, or Hotel)
...
add_10g_nic() {
  ...
  echo "             dhcp: false"
  echo "             addresses:"
  echo "               - 192.168.5.X/24  # CEPH public network"
}
